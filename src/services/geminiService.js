...
const {
  ...
  createUserPrompt,
  getFallbackProfile,
} = require("../prompts/musicMappingPrompt");
...
class GeminiService {
  constructor() {
    ...
    this.model = this.genAI.getGenerativeModel({
      ...
      // systemInstruction removed for Gemma compatibility
      generationConfig: {
        // responseMimeType removed for Gemma compatibility
    ...
    // Persistent filesystem cache for Gemini results
    this.initCache();
    console.log(`Gemini AI initialized with model: ${config.gemini.model}`);
  }

  /**
   * Initialize cache (simplified Map-based for now)
   * TODO: Fix cache-manager v5 API for persistent storage
   */
  async initCache() {
    ...
    console.log(
      `✓ In-memory cache initialized (TTL: ${ttlDays}d, Max: ${maxEntries})`
    );
    console.log(`⚠ Note: Persistent cache disabled temporarily - will be fixed`);
  }
  ...
  async analyzeBooksForMusic(bookData) {
    try {
      ...
      if (!geminiEnabled) {
        console.log(
          `⚠ Gemini disabled (ENABLE_GEMINI=false), using rule-based fallback`
        );
      ...
      // Check cache first (Map-based, synchronous)
      ...
      if (
        !musicProfile.primaryGenre ||
        !musicProfile.mood ||
        musicProfile.energy === undefined
      ) {
        throw new Error(
      ...
      if (
        typeof musicProfile.energy !== "number" ||
        musicProfile.energy < 0 ||
        musicProfile.energy > 10
      ) {
        throw new Error("Invalid energy value from Gemini");
      }

      console.log(
        `✓ Gemini recommendation: ${musicProfile.primaryGenre} / ${musicProfile.mood} (energy: ${musicProfile.energy})`
      );

      // Store in cache for future requests
      this.cache.set(cacheKey, musicProfile);
      console.log(
        `✓ Cached result for "${bookData.title}" (cache size: ${this.cache.size})`
      );
      ...
      if (
        ...
        console.error(
          "⚠ GEMINI QUOTA EXCEEDED - Consider setting ENABLE_GEMINI=false in .env"
        );
      }
